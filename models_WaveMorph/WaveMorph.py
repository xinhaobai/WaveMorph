...
import models_WaveMorph.configs_TransMorph as configs
...
# ===============================================
#   wavelet downsampling   # up 3d
# ===============================================
from models_WaveMorph.WiNet.DWT_IDWT.DWT_IDWT_layer import DWT_3D, IDWT_3D

class Down_wt3d(nn.Module):
    def __init__(self, in_ch, out_ch, isBN):
        super(Down_wt3d, self).__init__()
        self.isBN = isBN
        self.DWT = DWT_3D(wavename='haar')
        self.conv_bn_relu = nn.Sequential(
            nn.Conv3d(in_ch * 8, out_ch, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(out_ch),
            nn.ReLU(inplace=True), )
        self.conv = nn.Sequential(
            nn.Conv3d(in_ch * 8, out_ch, kernel_size=3, stride=1, padding=1), )
    def forward(self, x):
        yL, y_HL, y_LH, y_HH, y_LH2, y_HL2, y_HH2, y_HH3 = self.DWT(x)
        x = torch.cat([yL, y_HL, y_LH, y_HH, y_LH2, y_HL2, y_HH2, y_HH3], dim=1)
        if self.isBN:
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'x .print.shape: {x.shape}')# torch.Size([1, 96, 32, 32, 32])
        Wh, Ww, Wt = x.size(2), x.size(3), x.size(4)
        ...
        # print(f'x.print.shape: {x.shape}')# torch.Size([1, 32768, 96])

        outs = []
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, T, x, Wh, Ww, Wt = layer(x, Wh, Ww, Wt)  # downsample
            # print(f'x_out.print.shape: {x_out.shape}') # downsample finished
            ...
            if i in self.out_indices:
                ...
                outs.append(out)
                # print(f'x.print.shape: {x.shape}')
                # print(f'out.print.shape: {out.shape}')
        return outs
...
class ConvBlock(nn.Module):
    def __init__( self, in_channels, out_channels, stride, norm_type, down_sample, down_type, dim=2 ):
        ...
        if dim == 3:
            ...
            self.conv = nn.Conv3d( in_channels, out_channels, kernel_size=1, stride=stride, padding=0 )
            # downsample
            if self.down_type =='maxpool':
                self.down = nn.MaxPool3d(2)# test in v0
            elif self.down_type == 'avepool':
                self.down = nn.AvgPool3d(2)#
            elif self.down_type=='wavedown':
                self.down = WaveDown(out_channels, out_channels, True)#  √
...
class WaveDown(nn.Module):
    def __init__(self, in_ch, out_ch, isBN):
        super(WaveDown, self).__init__()
        self.isBN = isBN
        self.DWT = DWT_3D(wavename='haar')
        self.conv_bn_relu = nn.Sequential(
            nn.Conv3d(in_ch * 8, out_ch, kernel_size=1, stride=1, padding=0),
...
class WaveDownConvBlock(nn.Module):
    def __init__( self, in_channels, out_channels, stride, norm_type, down_sample=True, dim=2 ):
        super(WaveDownConvBlock, self).__init__()
        self.norm_type = norm_type
        self.down_sample = down_sample
        if dim == 3:
            if norm_type == "none":
            ...
            # wave down
            self.wave_down = WaveDown(out_channels, out_channels, True)#  √
        self.activation = nn.ReLU(out_channels) # √
    ...
    def forward(self, x):
        ...
        out = self.activation(out)
        if self.down_sample:
            out = self.wave_down(out)
        return out



class WaveDownConvBlock3_(nn.Module):
    def __init__( self, in_channels, out_channels, stride, norm_type, down_sample=True, dim=2 ):
        super(WaveDownConvBlock3_, self).__init__()
        self.norm_type = norm_type
        self.down_sample = down_sample
        if dim == 3:
            if norm_type == "none":
...
class UNet(nn.Module):
    def __init__(self, config, in_channel, n_classes, start_channel, wavelet='haar', dwt_fn=1, up_train=True):
        self.in_channel = in_channel
        self.n_classes = n_classes
        self.ch = start_channel
        act = "leakyrelu"# org

        norm_dec = 'instance'#'instance'
        ...
        dim = 3
        self.enco_0 = WaveDownConvBlock(self.ch * 4, self.ch * 8, 1, norm_type, False, dim)
        self.enco_0_ = WaveDownConvBlock(self.ch * 8, self.ch * 4, 1, norm_type, True, dim)
        self.mlpconv_0 = Mlp_conv( self.ch * 8, self.ch * 8, self.ch * 8 )

        self.enco_1 = WaveDownConvBlock(self.ch * 4, self.ch * 8, 1, norm_type, False, dim)
        self.enco_1_ = WaveDownConvBlock(self.ch * 4, self.ch * 4, 1, norm_type, True, dim)
        self.mlpconv_1 = Mlp_conv( self.ch * 8, self.ch * 4, self.ch * 8 )

        self.enco_2 = WaveDownConvBlock(self.ch * 4, self.ch * 4, 1, norm_type, False, dim)
        self.enco_2_ = WaveDownConvBlock(self.ch * 4, self.ch * 4, 1, norm_type, True, dim)
        self.mlpconv_2 = Mlp_conv( self.ch * 4, self.ch * 4, self.ch * 4 )

        self.enco_3 = WaveDownConvBlock( self.ch * 4, self.ch * 4, 1, norm_type, False, dim)
        self.enco_3_ = nn.Sequential(nn.InstanceNorm3d(self.ch * 4),
                                     nn.ReLU(self.ch * 4))

        self.mlpconv_3 = Mlp_conv( self.ch * 4, self.ch * 4, self.ch * 4 )
        #
        self.proj = nn.Conv3d(8*self.in_channel, self.ch * 4, kernel_size=1, stride=1, padding=0)
        # self.proj3 = nn.Conv3d( 2,            self.ch * 4, kernel_size=1, stride=1, padding=0)
        # self.proj2 = nn.Conv3d( self.ch * 4,  self.ch * 4, kernel_size=1, stride=1, padding=0)
        # self.proj1 = nn.Conv3d( self.ch * 4,  self.ch * 8, kernel_size=1, stride=1, padding=0)
        # self.proj0 = nn.Conv3d( self.ch * 8,  self.ch * 8, kernel_size=1, stride=1, padding=0)
        ...
        from models_WaveMorph import losses
        self.smooth = losses.smoothloss  # (output[1], y)

        # Diff from GropuMorph
        from GroupMorph.Model.Function import VecInt, upsample
        from GroupMorph.Model.Function import SpatialTransformer as SpatialTransforFlow
        self.VecInt = VecInt(7)
        self.UpsampleFlow = upsample
        self.SpatialTransforFlow = SpatialTransforFlow()


        self.convbnrelu1 = nn.Sequential(nn.Conv3d(self.ch*4*2, self.ch*4, 1, 1, 0,),
                                   nn.InstanceNorm3d(self.ch*4),
                                   nn.ReLU(inplace=True),
                                   )
        self.convbnrelu2 = nn.Sequential(nn.Conv3d(self.ch*8*2, self.ch*8, 1, 1, 0,),
                                   nn.InstanceNorm3d(self.ch*8),
                                   nn.ReLU(inplace=True),
                                   )
    ...
    def cat_h(self, LLH, LHL, LHH, HLL, HLH, HHL, HHH, LLH_, LHL_, LHH_, HLL_, HLH_, HHL_, HHH_, scale=2):
        LLH, LHL, LHH, HLL, HLH, HHL, HHH = self.split_ch(
            self.up(torch.cat([LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1), scale), 7)
    ...
    def forward(self, x, multi=False):
        ...
        # waveCNN Encoder
        #==========================================
        # cut = self.proj3(self.down(x))
        e0 = self.DWT(x)
        e1 = torch.cat(( e0[0], e0[1], e0[2], e0[3],
                         e0[4], e0[5], e0[6], e0[7] ), 1)
        e1 = self.proj(e1)
        e1 = self.enco_3_(e1)#
        e1 = self.enco_3(e1)
        e1 = self.mlpconv_3(e1) + e1
        #
        # cut = self.proj2(self.down(e1))
        e2 = self.enco_2_(e1)
        e2 = self.enco_2(e2)
        e2 = self.mlpconv_2(e2) + e2
        #
        # cut = self.proj1(self.down(e2))
        e3 = self.enco_1_(e2)
        e3 = self.enco_1(e3)
        e3 = self.mlpconv_1(e3) + e3
        #
        # cut = self.proj0(self.down(e3))
        e4 = self.enco_0_(e3)
        e4 = self.enco_0(e4)
        e4 = self.mlpconv_0(e4) + e4
        #==========================================
        #  waveCNN Encoder
        #==========================================
        ...
        #  smooth 1
        smo = 0
        LLL_8_4 = self.UpsampleFlow(LLL_8, is_flow=False)
        xy_LLL_1_4 = self.SpatialTransforFlow(LLL_8_4, xy_LLL_1_4) + xy_LLL_1_4
        ...
        #  smooth 2
        xy_LLL_1_2_4 = self.UpsampleFlow(xy_LLL_1_4, is_flow=True)
        xy_LLL_1_2 = self.SpatialTransforFlow(xy_LLL_1_2_4, xy_LLL_1_2) + xy_LLL_1_2
        ...
        #  smooth 2
        xy_LLL_1_2_1 = self.UpsampleFlow(xy_LLL_1_2, is_flow=True)
        xy_LLL_1_1 = self.SpatialTransforFlow(xy_LLL_1_2_1, xy_LLL_1_1) + xy_LLL_1_1

        #  add DiffeomorphicTransform
        f_xy = xy_LLL_1_1  # DVF


        out = self.spatial_trans(source, f_xy)  #   org
...
CONFIGS = {
    ...
    'TransMorph-Small128': configs.get_3DTransMorphSmall128_config(),
